...
- **Enable the repository hooks once per clone:** `git config core.hooksPath .githooks`. The hook runs `scripts/check-rule-map.py` when rule-map inputs are staged; use `.githooks/pre-commit --all` for a deliberate full check. It fails if a rule file is unreachable, a pointer is broken, or the root map exceeds its context budget. Git does not enable tracked hook directories automatically.
